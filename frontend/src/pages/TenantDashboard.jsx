    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <span className="px-2.5 py-0.5 rounded-full text-xs font-bold font-body bg-cream text-charcoal/50">
            {filtered.length}
          </span>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-3xl py-24 text-center"
          ...
          <p className="font-body text-sm text-charcoal/40">
            Browse properties and click "Request Rent" to get started.
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {filtered.map((r) => {
            ...
            const img = r.property?.images?.[0]?.url
              || r.property?.images?.[0]
              || "https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?w=400&q=80";
            return (
              <div key={r._id}
                className="bg-white rounded-2xl overflow-hidden flex transition-all hover:shadow-card-hover"
                style={{ border: "1px solid rgba(26,26,46,0.07)", boxShadow: "0 2px 12px rgba(26,26,46,0.05)" }}>
                <div className="w-36 flex-shrink-0 cursor-pointer"
                  onClick={() => r.property && onViewDetails?.(r.property)}>
                  <img src={img} alt="" className="w-full h-full object-cover"
                    onError={(e) => {
                      e.target.src = "https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?w=400&q=80";
                    }} />
                </div>
                <div className="flex-1 p-5 min-w-0">
                  <div className="flex items-start justify-between gap-3 mb-2">
                    ...
                    <span
                      className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold font-body flex-shrink-0 capitalize"
                      style={{ background: st.bg, color: st.color }}>
                      <span className="w-1.5 h-1.5 rounded-full" style={{ background: st.dot }} />
                      {r.status}
                    </span>
                  </div>
                  <p className="font-body text-sm text-charcoal/45 mb-2">
                    📍 {r.property?.location?.address}, {r.property?.location?.city}
                  </p>
                  <div className="flex items-center gap-4 font-body text-xs text-charcoal/40">
                    <span className="font-semibold text-orange text-sm">
                      ₹{r.property?.price?.toLocaleString("en-IN")}/mo
                    </span>
                    {r.moveInDate && (
                  ...
                  {r.message && (
                    <p className="font-body text-xs text-charcoal/35 mt-1.5 italic truncate">"{r.message}"</p>
                  )}
                </div>
                <div className="flex items-center px-5 flex-shrink-0">
                  ...
                  {r.status === "approved" && (
                    <div className="text-center">
                      ...
                      <div className="font-body text-xs font-bold text-green-600">Approved!</div>
                    </div>
                  )}
                </div>
              </div>
...
export default function TenantDashboard({ user, onLogout }) {
  ...
  // ── User-scoped localStorage key so saves never bleed between accounts ──
  const userId   = user?._id || user?.id || "guest";
  const SAVED_KEY = `hv_saved_${userId}`;
  ...
  // ── Load saved properties from localStorage on first render ─────────────
  const [savedProperties, setSavedProperties] = useState(() => {
    try {
      const stored = localStorage.getItem(`hv_saved_${
        (() => {
          try {
            const u = JSON.parse(localStorage.getItem("hv_user") || "{}");
  ...
  // ── Build a Set of saved IDs for fast lookup by PropertyCard/PropertyGrid ─
  const savedIds = new Set(savedProperties.map((p) => p._id));
  ...
  const handleTabChange = (tab) => {
    ...
    setActiveTab(tab);
    if (tab === "browse") {
      // If already on browse, scroll straight to properties
      setTimeout(() => {
  ...
  const handleSaveToggle = (property, isSaved) => {
    setSavedProperties((prev) => {
      let updated;
      if (isSaved) {
        updated = prev.find((p) => p._id === property._id)
          ? prev
          : [...prev, property];
      } else {
      ...
      try {
        ...
        console.error("Could not persist saved properties:", err);
      }
      return updated;
    });
  };
  ...
  return (
    <div className="min-h-screen bg-cream">
      ...
      <main
        ...
        style={{ paddingTop: activeTab !== "browse" ? "96px" : "48px" }}
      >
        {/* ── BROWSE TAB ── */}
        {activeTab === "browse" && (
          <div ref={browseTopRef} id="properties-section">
          ...
          <div className="max-w-[1200px] mx-auto">
            <div className="flex items-center gap-3 mb-8">
              ...
              {savedProperties.length > 0 && (
                <span
                  className="px-2.5 py-0.5 rounded-full text-xs font-bold font-body text-white"
