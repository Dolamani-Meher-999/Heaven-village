...
export default function TenantNavbar({ activeTab, setActiveTab, onLogout }) {
  ...
  const navItems = [
    { label: "Browse",      key: "browse",   icon: "⊞" },
    { label: "Saved",       key: "saved",    icon: "♡" },
    { label: "My Requests", key: "requests", icon: "📋" },
  ];
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              onMouseEnter={(e) => {
                if (!isActive) {
                  e.currentTarget.style.color = "rgba(26,26,46,1)";
            ...
            >
              <span style={isActive ? { filter: "brightness(10)" } : { color: "#E8621A" }}>
                {item.icon}
              </span>
              {item.label}
            </button>
      ...
      <div className="flex items-center gap-3" ref={dropdownRef}>
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2.5 bg-gray-50 border border-gray-100 rounded-xl px-3 py-2 hover:border-orange/25 transition-all duration-200"
          >
            <div className="w-7 h-7 rounded-lg orange-gradient flex items-center justify-center text-white font-bold text-sm">
              {user?.name?.[0]?.toUpperCase() || "T"}
            </div>
            <div className="text-left">
              <div className="font-body text-sm font-semibold text-charcoal leading-none mb-0.5">
          ...
          {dropdownOpen && (
            <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-gray-100 rounded-xl shadow-[0_20px_60px_rgba(26,26,46,0.12)] overflow-hidden">
              {[
                { icon: "♡",  label: "Saved Properties", key: "saved"    },
                { icon: "📋", label: "My Requests",       key: "requests" },
              ].map((item) => (
                <button
                  key={item.label}
                  onClick={() => { handleNavClick(item.key); setDropdownOpen(false); }}
