...
export default function PropertyGrid({ filters, onRequestRent, onSaveToggle, onViewDetails }) {
    ...
    <div className="flex-1 min-w-0">
      <div className="flex items-center justify-between mb-5">
        <p className="font-body text-charcoal/50 text-sm">
          {loading ? "Loading…" : (
            <span>
              Showing <span className="font-semibold text-charcoal">{properties.length}</span>
              {total > properties.length && <span> of <span className="font-semibold text-charcoal">{total}</span></span>}
              {" "}properties
            </span>
          )}
        </p>
      ...
      {error && (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <button onClick={() => fetchProperties(1)}
            className="orange-gradient px-6 py-2.5 rounded-xl font-body text-sm font-bold text-white shadow-orange">
            Try Again
          </button>
        </div>
      )}
      ...
      {!loading && !error && sorted.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="font-body text-charcoal/40 text-sm">Try adjusting your filters</p>
        </div>
      )}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-10">
              <button onClick={() => fetchProperties(page - 1)} disabled={page === 1}
                className="px-4 py-2 rounded-xl bg-white border border-gray-200 font-body text-sm text-charcoal/60 hover:border-orange/30 hover:text-orange transition-all disabled:opacity-30 disabled:cursor-not-allowed">
                ← Prev
              </button>
